...
let content = fs.readFileSync('resources/js/Pages/Settings/Automations.jsx', 'utf8');
...
content = content.replace("const MermaidDiagram = ({ flow }) => {", componentsCode + "\nconst MermaidDiagram = ({ flow }) => {");
...
const mainViewAnchor = "            {activeTab === 'flows' ? (";
const mainViewReplacement = \`            {activeTab === 'global' ? (
                <GlobalMermaidDiagram automations={automations} />
            ) : activeTab === 'simulator' ? (
                <SimulationPanel automations={automations} />
            ) : activeTab === 'flows' ? (\`;

content = content.replace(mainViewAnchor, mainViewReplacement);

fs.writeFileSync('resources/js/Pages/Settings/Automations.jsx', content, 'utf8');
